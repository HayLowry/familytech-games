...
function Board() {
  ...
  function handleSquareInput(letter, row, col, inputLocation, event) {
    let newBoard = board;
    newBoard[row].CURRENT_ROW[col].CHARACTER = letter;
    setBoard(newBoard);
    //Check letter to see if it's correct
    if (board[row].CURRENT_ROW[col].CHARACTER.toLowerCase() == board[row].CURRENT_ROW[col].KEY_CHARACTER.toLowerCase()) {
  ...
  function insertAllWords(
    ...
    while (!finished && wordsAdded < 25) {
      ...
      if (remainingWords.length < 1 || attemptedInsertions > 100 || addedWords >= 25) {
        finished = true;
      }
      if (finished) {
